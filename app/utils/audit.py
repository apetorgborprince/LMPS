...
def log_action(user_id, action, entity_type=None, entity_id=None, result="SUCCESS"):
    """Write an audit event through the authenticated Supabase session.

    The database RLS policy requires actor_user_id to be the current app user.
    """
    token = session.get("supabase_access_token")
    if not token:
        return None
    client = client_for_access_token(token)
    payload = {
